...
function Map:init()
    ...
    while x < self.mapWidth  do
        ...
        if x == self.mapWidth - 4 then
            ...
            self:setTile(x , self.mapHeight/2 - 4 , 14)
            for y = self.mapHeight / 2, self.mapHeight do
                self:setTile(x, y, TILE_BRICK)
            end
            x = x+1
        elseif x > self.mapWidth - 15 and x <self.mapWidth then
            if x == self.mapWidth - 14 then 
                self:setTile(x , self.mapHeight/2 - 1 , TILE_BRICK)
                for y = self.mapHeight / 2, self.mapHeight do
                    self:setTile(x, y, TILE_BRICK)
                end
                x = x+1
            elseif x == self.mapWidth - 13 then 
                for y = 1 , 2 do
                ...
                x = x+1
            elseif x == self.mapWidth - 12 then 
                for y= 1 , 3 do
                    self:setTile(x , self.mapHeight/2 -y , TILE_BRICK)
                end
                for y = self.mapHeight / 2, self.mapHeight do
                ...
                x = x+1
            elseif x == self.mapWidth - 11 then 
                for y= 1 , 4 do 
                    self:setTile(x , self.mapHeight/2 -y , TILE_BRICK)
                end
                for y = self.mapHeight / 2, self.mapHeight do
        ...
        elseif  x >= self.mapWidth - 20 and x < self.mapWidth - 15  then
            for y = self.mapHeight / 2, self.mapHeight do
                self:setTile(x, y, TILE_BRICK)
            end
            x = x+1
