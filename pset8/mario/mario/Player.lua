...
function Player:FlagRightCollision()
    if self.dx > 0 then
        -- check if there's a tile directly beneath us
        if self.map:flagCollides(self.map:tileAt(self.x + self.width, self.y)) or
            self.map:flagCollides(self.map:tileAt(self.x + self.width, self.y + self.height - 1)) then
